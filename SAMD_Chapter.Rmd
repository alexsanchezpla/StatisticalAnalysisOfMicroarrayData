...
Microarray data analysis is one of the most clear cases where interaction between bioinformatics and statistics has been  highly benefical for both disciplines. Efron @Efron2013 even calls the 21st century as the century of microarrays. 
...
Note that we have described the row contents as "genes". Strictly speaking depending on the type of array, each row may correspond to one "probeset" or one transcript.

- A probeset is, as indicated by its name, a set of "probes", which are designed to map different fragments of a given gene. Altogether it is expected that each set of probes, or probeset, uniquely characterizes one gene. Arrays may contain several distinct probesets for some genes because it is not always possible to do such characterization with a single probeset.
- A transcript describes how the gene has been transcribed into mesenger RNA. Due to the phenomenon of alternative splicing @Sanchez-Pla2012 there may be different transcriptions, i.e. different transcripts, for each gene.

In practice, given that both probesets or transcripts map to genes it is common to describe the array rows as "genes".
...
A popular user friendly interface to work with R is R-Studio. It can be downloaded and installed following the instructions listed in its web page: https://www.rstudio.com/. R-studio version used for this analysis was 1.1.383.
...
After creating the folders we save into the main folder the file with the microarray analysis script, that can be downloaded from [https://github.com/alexsanchezpla/StatisticalAnalysisOfMicroarrayData](https://github.com/alexsanchezpla/StatisticalAnalysisOfMicroarrayData). Once it is saved, you can open R-Studio, open the file with the R code, and in R-Studio go to the menu option in the top "Session -> Set working directory -> To source file location". This action will set the folder we have set as "main" folder as our working folder.
...
```{r targetsAspect, fig.cap="Aspect of the targets file for this study", echo=FALSE}
knitr::include_graphics("figures/Screenshot_targets.png")
```

Although the targets file need not have any fixed names it is pracical to use its column name sto create labels that will be used later. For example:
...
if (!require(oligo)) biocLite("oligo") else detach(package:oligo)
if (!require(pd.mogene.2.1.st)) biocLite("pd.mogene.2.1.st") else detach(package:pd.mogene.2.1.st)
if (!require(arrayQualityMetrics)) biocLite("arrayQualityMetrics") else detach(package:arrayQualityMetrics)
if (!require(colorspace)) install.packages("colorspace") else detach(package:colorspace)
if (!require(ggplot2)) install.packages("ggplot2") else detach(package:ggplot2)
if (!require(ggrepel)) install.packages("ggrepel") else detach(package:ggrepel)
if (!require(limma)) biocLite("limma") else detach(package:limma)
if (!require(genefilter)) biocLite("genefilter") else detach(package:genefilter)
if (!require(xtable)) install.packages("xtable") else detach(package:xtable)
if (!require(mogene21sttranscriptcluster.db)) biocLite("mogene21sttranscriptcluster.db")else detach(package:mogene21sttranscriptcluster.db)
if (!require(annotate)) biocLite("annotate") else detach(package:annotate)
if (!require(org.Mm.eg.db)) biocLite("org.Mm.eg.db") else detach(package:org.Mm.eg.db)
if (!require(ReactomePA)) biocLite("ReactomePA") else detach(package:ReactomePA)
if (!require(gplots)) install.packages("gplots") else detach(package:gplots)
```
...
```{r QCRawDataRes, fig.cap="Aspect of a screenshot of arrayQualityMetrics results (index.html file)", echo=FALSE}
...
```{r PCARaw, message=FALSE, fig.align='center', fig.height=6, fig.width=6, fig.cap="Visualization of the two first Principal Components for raw data"}
...
```{r QCNormDataRes, fig.cap="Aspect of a screenshot of arrayQualityMetrics results (index.html file) for the normalized data", echo=FALSE}
...
```{r BoxplotNorm, message=FALSE, fig.align='center', fig.height=6, fig.width=6, fig.cap="Distribution of  intensities for normalized data"}
...
Note that after normalization, intensity of arrays is very similar.
...
After filtering there are `r dim(exprs(eset_filtered))[1]` probesets left. Note that we have stored the probesets left in the variable *eset_filtered*
...
Annotation makes the tables more comprehensible. The table below shows the annotations added to the first top table (where only the first four columns are shown).
...
In our study for comparison _KOvsWT.COLD_ only a single enriched pathway has been found, _Synthesis of very long-chain fatty acyl-CoAs_, and in comparison _KOvsCTL.RT_, four enriched pathways have been found. The more statistically relevant is _ Inositol phosphate metabolism_.
...
```{r listOfFiles, echo=FALSE, results='hide'}
...
1. Sometimes package installation may present some difficulties, often related to the operanting system being used or even the workplace (for example if the user is behindd a proxy). In these cases the recommendation is to try to install packages one by one and, if needed contact the institution IT team.
...
3. R and Bioconductor are open source software. This mean they are free (as in "free bier") but it also means that compatibility between versions is not always 100% granted. It is important to know which version of R and Bioconductor one is using for the analysis.
