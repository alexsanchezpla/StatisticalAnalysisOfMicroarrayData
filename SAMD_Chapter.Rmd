...
```{r readPackagesList, results='hide', echo=FALSE}
packages4MDA <- read.csv("packages4MDA.txt")
```

```{r packagesList, echo=FALSE}
knitr::kable(
...
The following commands create the desired folders. This can be made from within R as descibed, or using a visual file browser such as Windows File Explorer or any other (in that case you can omit this step):
...
## 2.2 Prepare the data for the analysis
...
## 2.3 Packages installation in R
...
Standard R packages can be downloades and installed form default repositories with the `install.packages` function. 
Bioconductor packages can be downloaded and installed with the function `biocLite()` which at its time can be loaded into R with the instruction `source("https://bioconductor.org/biocLite.R")`. 
...
## 2.5 __Quality control of raw data__
...
## 2.7 __Quality control of normalized data__
...
## 2.9 __Detecting most variable genes__
...
## 2.10 __Filtering least variable genes__
...
## 2.11 __Saving normalized and filtered data__
...
## 2.12 __Defining the experimental setup: The design matrix__
...
## 2.13 __Defining comparisons with the Contrasts Matrix__ 
...
## 2.14 __Model estimation and gene selection__
...
## 2.15 __Obtaining lists of differentially expressed genes__
...
## 2.20 __Biological Significance of results__
...
In our study for comparison _KOvsWT.COLD_ only a single enriched pathway has been found, _Synthesis of very long-chain fatty acyl-CoAs_, and in comparison _KOvsCTL.RT_, four enriched pathways have been found (Table \@ref(tab:tableReacto)). The more statistically relevant is _Inositol phosphate metabolism_.
